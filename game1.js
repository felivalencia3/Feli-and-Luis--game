...
function preload() {
  ...
  this.load.html("form", "assets/coordinates.html")
}
...
function create() {
  ...
  food.depth = 12
  this.element = this.add.dom(580, 450).createFromCache('form');
  this.element.addListener('click');
  this.element.on('click', function (event) {
    if (event.target.name === 'GoButton') {
      moves += 1;
      moveText.setText("moves: "+moves)
      newY = 2*Number.parseInt(this.getChildByName('y').value);
      newX = 2*Number.parseInt(this.getChildByName('x').value);
